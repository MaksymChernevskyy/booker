...
public interface BookingRepository extends JpaRepository<Booking, Long> {
  ...
  List<Booking> getAllBookingsInPastAndRoom(@Param("bookedTo") LocalDateTime bookedTo, @Param("roomName") String roomName);

  @Query("select booking from Booking booking where ?1  > booking.bookedFrom and booking.bookedTo = ?2")
  List<Booking> getAllBookingsInFutureAndRoom(@Param("bookedFrom") LocalDateTime bookedFrom, @Param("roomName") String roomName);
